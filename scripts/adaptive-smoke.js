...
async function checksA(page, url, checks) {
  ...
  // A6 - Heading hierarchy removed (non-critical)
  ...
  checks.push({name:'Canonical Tag',pass:!!can,detail:can?can.substring(0,60):'No canonical — may cause duplicate content',improvement:true});
  ...
  checks.push({name:'Structured Data (Schema)',pass:schema,detail:schema?'JSON-LD found':'No schema — add for rich search results',improvement:true});
...
async function checksB(page, url, checks) {
  ...
  checks.push({name:'CTA Above Fold',pass:cta.above>0,detail:cta.above>0?`"${cta.text}"`:cta.total>0?`${cta.total} CTA(s) but none above fold`:'No CTA buttons found'});
    ...
    // B6 - Hero section — class, background image, or large above-fold element
  const heroOk = await page.evaluate(() => {
    // Class-based
    if (document.querySelector('.hero,.banner,.jumbotron,[class*="hero"],[class*="banner"],.wp-block-cover,.elementor-section:first-of-type')) return true;
    // Background image above fold
  ...
  checks.push({name:'Hero Section',pass:heroOk,detail:heroOk?'Hero or banner section found above fold':'No hero section detected — consider adding an engaging above-fold section'});
...
// B7 - Image alt text
  ...
  checks.push({name:'Social Media Links',pass:soc.length>0,detail:soc.length>0?`Found: ${soc.join(', ')}`:'No social media links found',improvement:true});
...
async function checksC(page, url, checks) {
  ...
  checks.push({name:'External Links Open New Tab',pass:ext.noTarget===0,detail:ext.noTarget===0?`All ${ext.total} external links use target="_blank"`:`${ext.noTarget}/${ext.total} external links missing target="_blank"`,improvement:true});
    ...
    // C4 - Nav active state — checks classes AND visual colour differences
  const hasActiveState = await page.evaluate(() => {
    const nav = document.querySelector('nav, header, .navbar, .navigation, .menu, #menu, #nav');
    if (!nav) return false;
    // Check for standard active classes
    if (nav.querySelector('.active,.current,[aria-current="page"],.current-menu-item,.is-active,.selected')) return true;
    // Check if any nav link has visually distinct colour (colour-based active state)
    const links = [...nav.querySelectorAll('a')].filter(a => a.offsetParent !== null);
    if (links.length < 2) return true; // can't compare
    const colors = links.map(a => getComputedStyle(a).color);
    const unique = new Set(colors);
    if (unique.size > 1) return true; // different colours = active state indicated
    // Check font-weight difference
    const weights = links.map(a => getComputedStyle(a).fontWeight);
    return new Set(weights).size > 1;
  });
  checks.push({name:'Nav Active/Current State',pass:hasActiveState,detail:hasActiveState?'Active page state detected in nav (class or colour)':'No active state in navigation — users cannot tell where they are'});
...
// C5 - Skip to content
  ...
  // C8 - Print stylesheet removed (non-critical)
...
async function checksD(page, checks) {
  ...
  if(ac.count>0)checks.push({name:'Input Autocomplete',pass:ac.miss===0,detail:ac.miss===0?'Contact fields have autocomplete':`${ac.miss} field(s) missing autocomplete`,improvement:true});
  ...
  checks.push({name:'Base Font Size (≥14px)',pass:resizable,detail:resizable?'Base font size acceptable':'Body font too small — affects readability',improvement:true});
}
...
async function checksE(page, url, checks, consoleErrors) {
  ...
  checks.push({name:'Modern Image Formats (WebP)',pass:mf.total===0||(mf.mod/mf.total)>=0.3,detail:mf.total===0?'No images':mf.mod>0?`${mf.mod}/${mf.total} use WebP/AVIF`:`0/${mf.total} use modern formats — convert to WebP`,improvement:true});
  ...
  checks.push({name:'Image Lazy Loading',pass:ll.total<3||(ll.lazy/ll.total)>=0.4,detail:`${ll.lazy}/${ll.total} images lazy-loaded`,improvement:true});
  ...
  // E8 - Inline CSS removed (non-critical)
...
async function checksF(page, url, checks) {
  ...
  checks.push({name:'Analytics Detected',pass:ga,detail:ga?'Google Analytics/GTM found — ensure GDPR compliance':'No analytics detected',improvement:true});
}
...
async function checksH(browser, url, checks, evidence) {
  ...
  try{
    ...
    checks.push({name:'[Mobile] Theme Color Meta',pass:!!tc,detail:tc?`theme-color: ${tc}`:'No theme-color meta',improvement:true});
