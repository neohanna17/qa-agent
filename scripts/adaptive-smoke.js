...
async function checksB(page, url, checks) {
  ...
  const hero=await page.evaluate(()=>{
    if(document.querySelector('.hero,.banner,.jumbotron,[class*="hero"],[class*="banner"],.wp-block-cover,.elementor-section'))return true;
    return [...document.querySelectorAll('div,section,header')].some(el=>{
      ...
      if(img&&img.naturalWidth>400){const ir=img.getBoundingClientRect();if(ir.width>window.innerWidth*0.5)return true;}
      return!!el.querySelector('video,iframe[src*="youtube"],iframe[src*="vimeo"]');
    });
  });
  checks.push({name:'Hero Section',pass:hero,detail:hero?'Hero or banner found above fold':'No hero section detected'});
  ...
  checks.push({name:'Social Media Links',pass:soc.length>0,detail:soc.length>0?`Found: ${soc.join(', ')}`:'No social links — add to footer',improvement:true});
...
async function checksC(page, url, checks) {
  ...
  const active=await page.evaluate(()=>{
    const nav=document.querySelector('nav,header,.navbar,.navigation,.menu,#menu,#nav,[class*="nav-"],[class*="-nav"]');
    if(!nav)return true;
    if(nav.querySelector('.active,.current,[aria-current="page"],.current-menu-item,.is-active,.selected,.menu-item-active'))return true;
    const links=[...nav.querySelectorAll('a')].filter(a=>a.offsetParent!==null&&(a.innerText||'').trim());
    if(links.length<2)return true;
    const colors=links.map(a=>getComputedStyle(a).color);
    if(new Set(colors).size>1)return true;
    const bgs=links.map(a=>getComputedStyle(a).backgroundColor).filter(b=>b!=='rgba(0, 0, 0, 0)'&&b!=='transparent');
    if(new Set(bgs).size>1)return true;
    const weights=links.map(a=>getComputedStyle(a).fontWeight);
    return new Set(weights).size>1;
  });
  checks.push({name:'Nav Active/Current State',pass:active,detail:active?'Active page state detected in navigation':'No visual active state in navigation'});
...
async function checksF(page, url, checks) {
  ...
  const cookie=await page.evaluate(()=>{
    ...
    if(document.querySelector('[class*="cookie"],[class*="consent"],[id*="cookie"],[id*="consent"],[class*="gdpr"],[id*="gdpr"],[class*="cc-"],[class*="cookiebanner"],[class*="cookie-notice"]'))return true;
    return[...document.querySelectorAll('script[src]')].some(s=>s.src.toLowerCase().includes('cookie')||s.src.toLowerCase().includes('consent'));
  });
  checks.push({name:'Cookie Consent',pass:cookie,detail:cookie?'Cookie/GDPR consent handling detected':'No cookie consent detected — check if a popup loads via JS',improvement:true});
  ...
  checks.push({name:'Clickjacking Protection',pass:true,detail:'Check server response headers for X-Frame-Options',improvement:true});
...
async function saveResults(ns, results) {
  ...
  try {
    ...
    const runEntry = { ns, date, url: results.url, name: results.name, score: results.score, status: results.status, runAt: results.runAt };
    await firebaseRequest('POST', '/allRuns', runEntry);
